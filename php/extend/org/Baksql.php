...
class Baksql {
    ...
    public function __construct($config) {
        $config['path']="public/db_up/"; //默认目录 
        $config["sqlbakname"]=date("Y-m-d His",time()).md5(time()).".sql";//默认保存文件
        $this->config = $config;
    ...
    private function writeToFile($tables = array(), $ddl = array(), $data = array())
    {
        ...
        return file_put_contents($this->config['path'].$this->config['sqlbakname'], $str) ? ['code'=>1,'name'=>$this->config['sqlbakname'],'msg'=>'备份成功!花费时间' . round(microtime(true) - $this->begin,2) . 'ms'] : [code=>0,'msg'=>'备份失败!'];
    }
    /**
     * 设置要备份的表
     * @param array $tables
    ...
    public function restore($filename = '')
    {
        ...
        else
        {
            ...
            try
            {
                ...
                return['code'=>1,'msg'=>'还原成功!花费时间', round(microtime(true) - $this->begin,2) . 'ms']; ;
            }
            catch (PDOException $e)
            {
    ...
    public function get_filelist($Order = 0) {
        ...
        while (false !== ($filename = readdir($FilePath))) {
            if ($filename!="." && $filename!=".."){
                ...
                $FileAndFolderAyy[$i]['createtime'] = $this->getfiletime($filename);
                $FileAndFolderAyy[$i]['size'] = $this->getfilesize($filename);
            }
            
        }
        $Order == 1 ? sort($FileAndFolderAyy) : rsort($FileAndFolderAyy);
        return $FileAndFolderAyy;
    }
    public function delfilename($filename){
        $path=$this->config['path'].$filename;          
        if (@unlink($path)) {return ['code'=>1,'msg'=>'删除成功'];}
    }
}
